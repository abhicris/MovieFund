...
export default function NewFilmPlanPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await authenticatedFetch('/api/film-plans', {
        method: 'POST',
        body: JSON.stringify({
      ...
      if (data.success) {
        router.push(`/producer/film-plans/${data.data.id}`);
      } else {
        setError(data.error || 'Failed to create film plan');
      }
    } catch (err: any) {
      setError(err.message || 'Failed to create film plan');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="flex gap-4 pt-8 border-t border-black">
              ...
              >
                {loading ? 'Creating...' : 'Create Film Plan'}
              </button>
              <Link
                href="/producer/film-plans"
